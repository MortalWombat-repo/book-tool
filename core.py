...
@app.command()
def book(title: str, author: str, action: Annotated[Optional[str], typer.Argument()] = None, result: Annotated[Optional[int], typer.Argument()] = 10):
    response = requests.get(f"https://www.googleapis.com/books/v1/volumes?q=intitle:{title}+inauthor:{author}&maxResults={result}")
    data = response.json()
    ...
    for p in [title, authors, sentences, rating, ratingcount]:
        typer.echo(p)
...
@app.command()
def title(title: str, result: Annotated[Optional[int], typer.Argument()] = 10):
    response = requests.get(f"https://www.googleapis.com/books/v1/volumes?q=intitle:{title}&maxResults={result}")
    data = response.json()
    titles = []
    if data.get("items"):
...
@app.command()
def author(author: str, result: Annotated[Optional[int], typer.Argument()] = 10):
    response = requests.get(f"https://www.googleapis.com/books/v1/volumes?q=intitle:{author}&maxResults={result}")
    data = response.json()
    titles = []
    if data.get("items"):
        for item in data["items"]:
...
def view():
    ...
    wrapped_rows = []
    for row in rows:
        wrapped_row = [textwrap.fill(str(item), width=10) for item in row]
